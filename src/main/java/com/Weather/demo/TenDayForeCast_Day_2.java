...
public class TenDayForeCast_Day_2
{
    ...
    {	
    	...
    	intTempHigh = Integer.parseInt(JsonPath.read(body, "$.forecast.simpleforecast.forecastday[1].high.fahrenheit").toString());
    	intTempLow = Integer.parseInt(JsonPath.read(body, "$.forecast.simpleforecast.forecastday[1].low.fahrenheit").toString());
    	    	
    	strDate = JsonPath.read(body, "$.forecast.simpleforecast.forecastday[1].date.pretty").toString();
