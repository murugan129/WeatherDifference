...
public class TenDayForeCast_Day_1
{
    ...
    {	
    	...
    	intTempHigh = Integer.parseInt(JsonPath.read(body, "$.forecast.simpleforecast.forecastday[0].high.fahrenheit").toString());
    	intTempLow = Integer.parseInt(JsonPath.read(body, "$.forecast.simpleforecast.forecastday[0].low.fahrenheit").toString());
    	    	
    	strDate = JsonPath.read(body, "$.forecast.simpleforecast.forecastday[0].date.pretty").toString();
